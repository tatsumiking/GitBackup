...
CDialogPartSave::CDialogPartSave(CWnd* pParent /*=NULL*/)
	: CDialog(CDialogPartSave::IDD, pParent)
{
	...
	m_nLstMax = 0;
	m_hDib = 0;
	//{{AFX_DATA_INIT(CDialogPartSave)
	//}}AFX_DATA_INIT
}


void CDialogPartSave::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDialogPartSave)
...
BEGIN_MESSAGE_MAP(CDialogPartSave, CDialog)
	//{{AFX_MSG_MAP(CDialogPartSave)
	ON_WM_PAINT()
...
/////////////////////////////////////////////////////////////////////////////
// CDialogPartSave メッセージ ハンドラ

void CDialogPartSave::OnPaint() 
{
	CRect	rectImg;
...
BOOL CDialogPartSave::OnInitDialog() 
{
	CRect	rectDlg;
	CRect	rectImg;
	int		x, y, cx, cy;
...
void CDialogPartSave::OnBeforpage() 
{
...
void CDialogPartSave::OnNextpage() 
{
	m_nCrtTop += (m_nPageMax/2);
...
void CDialogPartSave::OnSelchangeLstimg() 
{
...
void CDialogPartSave::OnLButtonDown(UINT nFlags, CPoint point) 
{
	CRect	rect;
	int		idx;
	int		sx, sy, ex, ey;
...
void CDialogPartSave::LegalRect(CRect *rect)
{
	CRect	rectTop;
...
void CDialogPartSave::DrawPartImg(CDC *pcDC, CRect rect)
{
...
void CDialogPartSave::ImageDraw(CDC *pcDC, int sx, int sy, LPTSTR filename, ULONG nSize)
{
	CFile	cpFile;
	LPBYTE	lpDib;
	LPBYTE 	lpInfo;
...
void CDialogPartSave::InitListImage()
{
	TCHAR	szPartPath[FNAMEMAX];
	TCHAR	szFileName[FNAMEMAX];
	FILE	*fp;
...
void CDialogPartSave::CreatePartList()
{
	TCHAR	szPartPath[FNAMEMAX];
	TCHAR	szFileName[FNAMEMAX];
...
void CDialogPartSave::BoxsDraw(CDC *pcDC, int sx, int sy, int ex, int ey)
{
