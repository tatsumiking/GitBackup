...
UINT CCmdKamonCrct::ExecKamonLoad(UINT event, DBL dx, DBL dy)
{
	...
	int		ret;
	DBL		size;
	TCHAR	szkmnchar[QRECMAX];
	TCHAR	szkmnfont[RECMAX];
	...
	if(ret == IDOK){
		...
		_tcscpy_s(szkmnchar, pDialogKamonLoad->m_szKamonChar);
		_tcscpy_s(szkmnfont, pDialogKamonLoad->m_szKamonFont);
		szkmnfont[0] = _T('K');
...
void CCmdKamonCrct::KamonLoadSetVect(DBL size, LPTSTR fontname, LPTSTR lpch)
{
	CTonDoc*	pcDoc;
	UINT		code;
