...
CDialogWakuDelete::CDialogWakuDelete(CWnd* pParent /*=NULL*/)
	: CDialog(CDialogWakuDelete::IDD, pParent)
{
	m_pcView = (CTonView*)pParent;
	m_nFirstFlag = TRUE;
	m_nCrtTop = 0;
	m_hDib = 0;
	//{{AFX_DATA_INIT(CDialogWakuDelete)
		// メモ - ClassWizard はこの位置にマッピング用のマクロを追加または削除します。
	//}}AFX_DATA_INIT
}


void CDialogWakuDelete::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDialogWakuDelete)
...
BEGIN_MESSAGE_MAP(CDialogWakuDelete, CDialog)
	//{{AFX_MSG_MAP(CDialogWakuDelete)
	ON_WM_LBUTTONDOWN()
	ON_WM_PAINT()
...
/////////////////////////////////////////////////////////////////////////////
// CDialogWakuDelete メッセージ ハンドラ

void CDialogWakuDelete::OnPaint() 
{
	RECT	Rect;
...
void CDialogWakuDelete::WakuAllDraw(CDC *pcDC)
{
...
void CDialogWakuDelete::WakuDraw(CDC *pcDC, int sx, int sy, LPTSTR wakufname)
{
...
void CDialogWakuDelete::WakuImageDraw(CDC *pcDC, int sx, int sy, LPTSTR initfname, long nSize)
{
	CFile	cpFile;
	LPBYTE	lpDib;
	LPBYTE 	lpInfo;
...
void CDialogWakuDelete::WakuBoxsDraw(CDC *pcDC, DBL sx, DBL sy, LPTSTR wakufname)
{
	FILE	*fp;
	TCHAR	buf[RECMAX];
	TCHAR	*sp;
...
void CDialogWakuDelete::TrnsTimeSet(DBL paperwidth, DBL paperhight)
{
...
void CDialogWakuDelete::BoxsDraw(CDC *pcDC, long sx, long sy, long ex, long ey)
{
	pcDC->MoveTo(sx, sy);
	pcDC->LineTo(ex, sy);
	pcDC->LineTo(ex, ey);
...
void CDialogWakuDelete::OnLButtonDown(UINT nFlags, CPoint point) 
{
	int		sx, sy;
	int		idx;
	...
	while(1){
		...
		&& (sy-WAKUSPACE) <= point.y && point.y <= (sy+WAKUSIZE)){
			...
			if(AfxMessageBox(_T("枠を削除してよろしいですか"), MB_OKCANCEL) == IDCANCEL){
...
void CDialogWakuDelete::GetWakuCountMax()
{
	TCHAR	paperpath[FNAMEMAX];
	int		idx;
	TCHAR	wakulistfname[FNAMEMAX];
...
BOOL CDialogWakuDelete::OnInitDialog() 
{
...
void CDialogWakuDelete::OnBeforpage() 
{
...
void CDialogWakuDelete::OnNextpage() 
{
	m_nCrtTop = m_nCrtTop + m_nOneLineCount;
...
void CDialogWakuDelete::GetWakuSize(TCHAR *wenvfile, DBL *wakuxsize, DBL *wakuysize)
{
	FILE	*fp;
	TCHAR	buf[RECMAX];
...
void CDialogWakuDelete::DelCrtSlctWaku()
{
	TCHAR	paperpath[FNAMEMAX];
	TCHAR	wakuname[FNAMEMAX];
	...
	_tremove(filename);
	wakuenvlistdelwakuname(slctidx);
}

int CDialogWakuDelete::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	// ダイアログを最大化
...
void CDialogWakuDelete::OnSelchangeLstwakuname() 
{
