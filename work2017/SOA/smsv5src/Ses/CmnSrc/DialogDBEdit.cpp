...
void CDialogDBEdit::OnClickLstitem(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
	...
	if(m_nCrtCol == 0){
		...
		if(nCheck == TRUE){
			m_lstFileList.SetItemText(m_nCrtLine, 1, _T("0"));
		}else{
			m_lstFileList.SetItemText(m_nCrtLine, 1, _T("1"));
		}
	}
...
// カラム名設定
// フィールド名(選択非選択)
	// int len = _tcslen(_T("選択")) + 1;
	//_tcscpy_s(m_pcApp->m_lpTempStr, len _T("選択"));
	//m_lstFileList.InsertColumn(0, m_pcApp->m_lpTempStr, LVCFMT_LEFT, 20);
	m_pcApp->m_lpTempStr[0] = _T('\0');
...
// フィールド名(データベース)
	...
	if(fildmax > DBEDITFILEMAX){
		...
		::AfxMessageBox(_T("フィールド数が多すぎるため２５０以降のフィールドは処理できません"), MB_OK);
	}
...
int CDialogDBEdit::InitRecord()
{
	...
	while(1){
		if(line >= nRecMax){
			line = nRecMax;
			_stprintf_s(m_pcApp->m_lpTempStr, TEMPBUFMAX2, _T("レコード数が多すぎるため%d以降のレコードは処理できません"), nRecMax);
			::AfxMessageBox(m_pcApp->m_lpTempStr, MB_OK);
			break;
		}
		nRow = line;
		m_lstFileList.InsertItem(nRow+1, _T(" "));
		//m_lstFileList.SetCheck(nRow, TRUE);
		//m_lstFileList.SetItemText(nRow, 1, "0");
		m_lstFileList.SetItemText(nRow, 1, _T(""));
		ret = FALSE;
...
void CDialogDBEdit::GetFildName(int nFildNo, LPTSTR lpFildName)
{
	LVCOLUMN	cLvcolumn;
	int		nCol;
...
int CDialogDBEdit::GetFildElement(LPTSTR lpFildName, LPTSTR LPTSTR)
{
	int nFildNo = GetFildNo(lpFildName);
	if(nFildNo == -1){
		return(FALSE);
	}
	GetFildElement(nFildNo, LPTSTR);
	return(TRUE);
}

int CDialogDBEdit::GetFildNo(LPTSTR lpFildName)
{
	LVCOLUMN	cLvcolumn;
	int		nFildNo;
	int		nCol;
	...
	for(nFildNo = 0; nFildNo < m_nFildMax; nFildNo++){
		nCol = nFildNo+STARTCOL;
		m_lstFileList.GetColumn(nCol, &cLvcolumn);
		if(_tcscmp(lpFildName, szFildName) == 0){
			return(nFildNo);
		}
	}
	return(-1);
...
void CDialogDBEdit::GetFildElement(int nFildNo, LPTSTR LPTSTR)
{
	int		nCol;
	...
	nCol = nFildNo+STARTCOL;
	m_lstFileList.GetItemText(m_nCrtLine, nCol, LPTSTR, RECMAX8);
