...
void CCmdBase::HlpMsgDsp(LPTSTR msg)
{
	m_pcWnd->HelpTextOut(STATBARHELP, msg);
}

//P 指定位置にステータスバーに文字列を表示する関数
void CCmdBase::HlpMsgDsp(int no, LPTSTR msg)
{
...
void CCmdBase::DispSelectDataSize()
{
	...
	_stprintf_s((LPTSTR)msgbuf, HRECMAX, _T("ｻｲｽﾞ%4.1lfmm %4.1lfmm"), sizex, sizey);
	HlpMsgDsp(1, (LPTSTR)msgbuf);
}
...
void CCmdBase::ScrnAreaRedraw(DBL minx, DBL miny, DBL maxx, DBL maxy)
{
	...
	_stprintf_s(m_szMsgBuf, _T("%1.1lf %1.1lf %1.1lf %1.1lf\n"), minx, maxx, miny, maxy);
	TRACE(m_szMsgBuf);
