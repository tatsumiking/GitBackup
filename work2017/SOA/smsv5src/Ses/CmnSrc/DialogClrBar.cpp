...
void CDialogClrBar::PltListInit()
{
	...
	TCHAR	pltlistfname[FNAMEMAX];
	TCHAR	buf[RECMAX];
	TCHAR	*sp;
	TCHAR	pltname[HRECMAX];
	FILE	*fp;

	CComboBox* pCB = (CComboBox*)GetDlgItem(IDC_PLTCOMBO);
	pCB->AddString(_T("penvalue.env"));
	senvgetexepath(exepath);
	_stprintf_s(pltlistfname, _T("%s\\palet.lst"), exepath);
	_tfopen_s(&fp, pltlistfname, _T("r"));
	if(fp == NULL){
		return;
	}
	while(1){
	...
	fclose(fp);
	_stprintf_s(pltname, _T("penvalue.env"));
	senvsetcrtpltname(pltname);
	m_csPltFilename = pltname;
	UpdateData(FALSE);
}
...
void CDialogClrBar::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	for(idx = 0, plt = m_nCrtPltTopNo; idx < PLTMAX; idx++, plt++){
		...
		if(rect.PtInRect(point)){
			senvgetpenrgb(plt, &rval, &gval, &bval);
			_stprintf_s(buf, _T("(R%03d, G%03d, B%03d)%04d"), rval, gval, bval, plt);
