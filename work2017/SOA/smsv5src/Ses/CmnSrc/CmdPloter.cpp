...
:CCmdBase(pcview)
{
	...
	_stprintf_s(fname, _T("%s\\defenv\\cmdplout.env"), exepath);
	_tfopen_s(&fp, fname, _T("r"));
	if(fp != NULL){
		baselibCrDelFgets(buf, RECMAX, fp);
		_stscanf_s(buf, _T("%lf"), &m_dNoriSiro);
		baselibCrDelFgets(buf, RECMAX, fp);
		_stscanf_s(buf, _T("%d"), &m_nCheckFlag);
		baselibCrDelFgets(buf, RECMAX, fp);
		_stscanf_s(buf, _T("%d"), &m_nAreaBoxsOutFlag);
		baselibCrDelFgets(buf, RECMAX, fp);
		_stscanf_s(buf, _T("%d"), &m_nFileNo);
		baselibCrDelFgets(buf, RECMAX, fp);
		_stscanf_s(buf, _T("%d"), &m_nDataNo);
		fclose(fp);
	}
}
...
void CCmdPloter::DivPloterOutLoop()
{
	...
	for(i = 0; i < m_nDivYCnt; i++){
		...
		for(j = 0; j < m_nDivXCnt; j++){
			...
			ret = IDOK;
			if(m_nCheckFlag == ON){
				ret = AfxMessageBox(_T("ダイレクト出力しますか"), MB_OKCANCEL);
			}
			if(ret == IDOK){
