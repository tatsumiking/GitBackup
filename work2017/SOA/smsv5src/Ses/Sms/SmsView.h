...
public:
	...
	void PrintEnvSetElementLoad(LPTSTR filename);
	void AreaBackClrFill(CDC *pcDC);
	void ChangeFont(LPTSTR lpFont);
	...
	void AllWakuSetString(LPTSTR lpText);
	UINT CommandExec(UINT event, long x, long y);
	void FillWorkArea(CDC *pcDC);
	void InitArea(CRect rect);
	void PrintEnvSave(LPTSTR lpBinFile);
	void PrintEnvLoad(LPTSTR lpBinFile);
