...
extern TCHAR	exepath[FNAMEMAX+2];
...
DllExport void rulelibgetkind(int kindno, long *pattern, int *style, TCHAR *stylename)
{
	...
	*pattern = 0;
	*style = 0;
	_stprintf_s(fname, _T("%s\\ptnstyle.env"), exepath);
	...
	while(1){
		...
		_stscanf_s(buf, _T("%d %lx %d %s"), &no, &ptn, &type, &name, RECMAX);
		if(tno == kindno){
			*pattern = ptn;
			*style = type;
			len = _tcslen(name) + 1;
			_tcscpy_s(stylename, len, name);
		}
		tno++;
	}
	fclose(fp);
