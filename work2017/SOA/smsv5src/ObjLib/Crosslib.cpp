...
DllExport dbezetype crossTransFreecurveBezier(dcurvetype *pFc)
{
	dbezetype result;
...
DllExport dbezetype crossTransArcBezier(darctype *ac)
{
	int		stat;
	DBL		x0, y0;
	DBL		srag, erag, dr;
	...
	if(stat != FAIL){
		stat = curve3pragrget(ac->x1, ac->y1, ac->x2, ac->y2, ac->x3, ac->y3, x0, y0, &srag, &erag, &dr);
	}
	if(stat == FAIL){
