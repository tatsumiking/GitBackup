...
void CMX2Font::FontNameSet(LPTSTR pFont)
{
...
int CMX2Font::SF1TrnsDataLoad(int sjis)
{
	int		ret;
...
int CMX2Font::SF1TrnsDataLoadNON(int sjis)
{
	int		ret;
...
void CMX2Font::SF1TrnsDataSave(int sjis)
{
	TrnsCopyFSF1FBuf();
	DataSave(sjis);
}

int CMX2Font::DataLoad(int sjis)
{
	int	ku, ten;
...
int CMX2Font::DataSave(int sjis)
{
	int	ku, ten;
...
{
	_tfopen_s(&m_fileMX2, m_sBaseFname, _T("rb"));
	if(m_fileMX2 == NULL){
		return(FAIL);
	}
	return(0);
...
{
	_tfopen_s(&m_fileMX2, m_sBaseFname, _T("r+b"));
	if(m_fileMX2 == NULL){
		if(FontIdxInit() == FAIL){
			return(FAIL);
		}
...
int CMX2Font::FontIdxInit()
{
	...
	_tfopen_s(&m_fileMX2, m_sBaseFname, _T("wb"));
	if(m_fileMX2 == NULL){
		return(FAIL);
	}
	memset(buf, _T('\0'), MX2HEADBYTE);
...
void CMX2Font::FaceNameSet(LPTSTR name)
{
