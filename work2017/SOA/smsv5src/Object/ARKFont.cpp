...
void CARKFont::FontNameSet(LPTSTR pFont, int type)
{
	TCHAR	fontpath[FNAMEMAX];

	m_nType = type;
	mojienvgetfontpath(fontpath);
	_stprintf_s(m_sBaseFname, FNAMEMAX, _T("%s\\%s.fnt"), fontpath, pFont);
}
...
int CARKFont::SF1TrnsDataLoad(int sjis)
{
	int		ret;
...
int CARKFont::SF1TrnsDataLoadNON(int sjis)
{
	int		ret;
...
void CARKFont::SF1TrnsDataSave(int sjis)
{
	TrnsCopyFSF1FBuf();
	DataSave(sjis);
}

int CARKFont::DataLoad(int sjis)
{
	int	ku, ten;
...
int CARKFont::ArcDataLoad(int ku, int ten)
{
	...
	fread(&(arkidx.ku), sizeof(BYTE), 1, m_fileARK);
	fread(&(arkidx.ten), sizeof(BYTE), 1, m_fileARK);
...
int CARKFont::DataSave(int sjis)
{
	int	ku, ten;
...
int CARKFont::ArcDataSave(int ku, int ten)
{
	...
	fseek(m_fileARK, idxseekp, 0);
	fwrite(&(arkidx.ku), sizeof(BYTE), 1, m_fileARK);
	fwrite(&(arkidx.ten), sizeof(BYTE), 1, m_fileARK);
...
{
	_tfopen_s(&m_fileARK, m_sBaseFname, _T("rb"));
	if(m_fileARK == NULL){
		return(FAIL);
	}
	return(0);
...
{
	_tfopen_s(&m_fileARK, m_sBaseFname, _T("r+b"));
	if(m_fileARK == NULL){
		if(FontIdxInit() == FAIL){
			return(FAIL);
		}
...
int CARKFont::FontIdxInit()
{
	...
	_tfopen_s(&m_fileARK, m_sBaseFname, _T("wb"));
	if(m_fileARK == NULL){
		return(FAIL);
	}
	memset(buf, _T('\0'), ARKHEADBYTE);
...
void CARKFont::FaceNameSet(LPTSTR name)
{
