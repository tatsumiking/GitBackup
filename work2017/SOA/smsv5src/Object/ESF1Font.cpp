...
// Easy Scanner System
//
// object esf1font.cpp  ÇTÉoÉCÉgÇdÇrèëëÃèàóùä÷êî
//
...
void CESF1Font::FontNameSet(LPTSTR pFont)
{
	...
	len1 = _tcslen(_T("AR"));
	len2 = _tcslen(_T("ARâpêî"));
	if(_tcsnccmp(_T("AR"), pFont,len1) == 0){
		if(_tcsnccmp(_T("ARâpêî"), pFont, len2) == 0){
...
int CESF1Font::SF1TrnsDataLoad(int sjis)
{
	int		ret;
...
int CESF1Font::SF1TrnsGaijiDataLoad(int sjis)
{
	int		ret;
...
int CESF1Font::SF1TrnsDataSave(int sjis)
{
	TrnsCopyFSF1FBuf();
	UserDataSave(sjis);
	return(0);
}
int CESF1Font::SF1TrnsDataRealSave(int sjis)
{
	TrnsCopyFSF1FBuf();
	DataRealSave(sjis);
	return(0);
}
int CESF1Font::DataLoad(int sjis)
{
	int	ku, ten;
...
int CESF1Font::GaijiDataLoad(int sjis)
{
	int	ku, ten;
...
int CESF1Font::UserDataSave(int sjis)
{
	int			ku, ten;
...
int CESF1Font::DataRealSave(int sjis)
{
	int			ku, ten;
	long 		idxseekp, datseekp;
	int			datlen;
	...
	basesjtokuten(sjis, &ku, &ten);
	if (FontWriteRealOpen(ku, ten) == FAIL) {
		return(FAIL);
	}
...
int CESF1Font::UserDataExist(int sjis)
{
	int			ku, ten;
	long 		idxseekp, datseekp;
	int			datlen;
...
int CESF1Font::UserDataDelete(int sjis)
{
	int			ku, ten;
	long 		idxseekp, datseekp;
	int			datlen;
...
int CESF1Font::DataRealLoad(int ku, int ten)
{
	long 		idxseekp, datseekp;
	int			datlen;
	e1idxtype	f1idx;
...
{
	TCHAR	ffname[FNAMEMAX];

	if(ku < ESG1KUTOP){
		_stprintf_s(ffname, FNAMEMAX, _T("%s.uf1"), m_sBaseFname);
	}
	else{
		_stprintf_s(ffname, FNAMEMAX, _T("%s.ug1"), m_sBaseFname);
	}
	_tfopen_s(&m_fileSF1, ffname, _T("r+b"));
	if(m_fileSF1 == NULL){
		FontTblDatNullSet(ffname);
		_tfopen_s(&m_fileSF1, ffname, _T("r+b"));
		if(m_fileSF1 == NULL){
			return(FAIL);
		}
...
int CESF1Font::UserFontOpen(int ku, int ten)
{
	...
	_tfopen_s(&m_fileSF1, ffname, _T("rb"));
	if(m_fileSF1 == NULL){
		return(FAIL);
	}
	return(0);
...
int CESF1Font::FontRealOpen(int ku, int ten)
{
	TCHAR	ffname[FNAMEMAX];

	if(ku < ESG1KUTOP){
		_stprintf_s(ffname, _T("%s.sf1"), m_sBaseFname);
	}
	else{
		_stprintf_s(ffname, _T("%s.sg1"), m_sBaseFname);
	}
	_tfopen_s(&m_fileSF1, ffname, _T("rb"));
	if(m_fileSF1 == NULL){
		return(FAIL);
...
int CESF1Font::FontWriteRealOpen(int ku, int ten)
{
	TCHAR	ffname[FNAMEMAX];
	...
	_tfopen_s(&m_fileSF1, ffname, _T("w+b"));
	if (m_fileSF1 == NULL) {
		return(FAIL);
	}
	return(0);
...
int CESF1Font::FontNullSet()
{
	TCHAR	sFontFileName[FNAMEMAX];
	int ret;

	_stprintf_s(sFontFileName, _T("%s.sf1"), m_sBaseFname);
	ret = FontTblDatNullSet(sFontFileName);
	return(ret);
}
int CESF1Font::UserFontNullSet()
{
	...
	_stprintf_s(sFontFileName, _T("%s.uf1"), m_sBaseFname);
	ret = FontTblDatNullSet(sFontFileName);
	if (ret == FAIL) {
		return(FAIL);
	}
	_stprintf_s(sFontFileName, _T("%s.ug1"), m_sBaseFname);
	ret = FontTblDatNullSet(sFontFileName);
	return(ret);
}
int CESF1Font::FontTblDatNullSet(TCHAR *lpFontFileName)
{
	TCHAR	buf[10];
	long	len;
	...
	_tfopen_s(&m_fileSF1, lpFontFileName, _T("wb"));
	if (m_fileSF1 == NULL) {
		return(FAIL);
	}
	len = ESF1DSEEKP;
...
void CESF1Font::TateGakiTrns(int sjis)
{
	int		eofflag;
	int		pcnt, pp, atr;
	DBL		x, y, xx, yy;
...
void CESF1Font::TrnsESF1ToFSF1()
{
	...
	for(pp = 0; pp < pcnt; pp++){
		...
		if(EorChk(m_bFBuf, pp) == ON){
			...
			// ï÷ãXìIÇ…ï¬ê}å`ÇÃÇ›àµÇ¢Ç∆Ç∑ÇÈ
			// ïœçXÇÃç€ÇÕÉ\ÉAÇ≈égópÇµÇƒÇ¢ÇÈèëëÃÇ‡ïœçXÇ™ïKóv
...
int CESF1Font::DFileReadOpen(TCHAR basefname [ ])
{
	TCHAR	ffname[FNAMEMAX];

	_stprintf_s(ffname, _T("%s.sf1"), basefname);
	_tfopen_s(&m_fileSF1, ffname, _T("rb"));
	if(m_fileSF1 == NULL){
		return(FAIL);
	}
