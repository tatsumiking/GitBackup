...
IMPLEMENT_SERIAL(CWinTTFont, CObject, 3)
CWinTTFont::CWinTTFont()
{
}

CWinTTFont::CWinTTFont(int nDummy)
{
...
void CWinTTFont::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
		// ar << 
...
void CWinTTFont::FSF1ClassSet(CFSF1Font* pFSF1Font)
{
	m_pcFSF1Font = pFSF1Font;
}

void CWinTTFont::FontNameSet(LPTSTR pFont)
{
	_tcscpy_s(m_sCrtFont, pFont);
}

void CWinTTFont::BezeAddTimeSet(DBL time)
{
	m_dBezeaddtime = time;
}

void CWinTTFont::TrnsElementReset()
{
	m_dBezeaddtime = 1.3;
	m_dXaddtime = 1.0;
	m_dYaddtime = 1.0;
...
void CWinTTFont::SetTateYoko(int flag)
{
	m_nTateYoko = flag;
}

void CWinTTFont::SF1TrnsDataLoad(int code)
{
	HDC	hDC;
...
void CWinTTFont::DataLoad(HDC hDC, UINT sjis)
{
	GLYPHMETRICS	gm;
	MAT2	mt;
	HFONT	hF, oldhF;
	...
	hF = CreateFont(sizeh, sizew, 0, 0, bold, itric, 0, 0,
			//SHIFTJIS_TCHARSET,OUT_TT_PRECIS,
	...
	//if(mtr.tmMaxTCHARWidth < 255){
	// 2002.07.25 èºà‰â∆ñ‰ÇÃÇΩÇﬂÇ…î˜í≤êÆ
	len = _tcslen(_T("Kamon"));
	if(_tcsnccmp(m_sCrtFont, _T("Kamon"), len) == 0){
		time = 2.09;
		...
		//time = (DBL)(mtr.tmHeight) / (DBL)(mtr.tmAveCharWidth);
		time = 1.0;
		m_dXaddmove = 0;
		m_dYaddmove = mtr.tmDescent;
	}
...
#ifdef DBG
	_stprintf_s(dbgmsgbuf, "<%s> %x m_lGlyphLen %ld(0x%04x)", m_sCrtFont, sjis, m_lGlyphLen, m_lGlyphLen);
	hmsgdisp(dbgmsgbuf);
#endif

	if(m_lGlyphLen < 0){
		_stprintf_s(msgbuf, QRECMAX, _T("<%s> 0x%04XÇÕïœä∑Ç≈Ç´Ç‹ÇπÇÒ"), m_sCrtFont, sjis);
...
#ifdef DBG
	_stprintf_s(dbgmsgbuf, "Boxs %d %d Org %d %d Inc %d %d", 
...
void CWinTTFont::GlyphToFSF1Trns()
{
...
void CWinTTFont::PointSetStart()
{
	m_pcFSF1Font->PointSetStart();
}

void CWinTTFont::PointSetEnd()
{
	m_pcFSF1Font->PointSetEnd();
}

void CWinTTFont::MovetoSet(LPPOINTFX lpcood)
{
	DBL		dx, dy;
...
void CWinTTFont::PolygonSet(LPPOINTFX lpcood, WORD count)
{
	WORD	idx;
	DBL		x1, y1, x2, y2;
...
void CWinTTFont::ConicSet(LPPOINTFX lpcood, WORD count)
{
	WORD	idx;
...
void CWinTTFont::BezeSet(LPPOINTFX lpcood, WORD count)
{
	WORD	idx;
	double	x1, y1, x2, y2, x3, y3, x4, y4;
...
void CWinTTFont::PolyCurveSet(LPPOINTFX lpcood, WORD count)
{
	WORD	idx;
...
void CWinTTFont::B_splinePointSet(DBL x1, DBL y1, DBL x2, DBL y2, DBL x3, DBL y3)
{
	DBL		t, t1, b1, b2, b3;
	DBL		tx, ty;
...
void CWinTTFont::TTFcoodToSF1cood(DBL *retx, DBL *rety)
{
	DBL		tx, ty, dx, dy;
...
void CWinTTFont::ParentSet(CWnd *pParent)
{
 	m_pcParent = pParent;
}
