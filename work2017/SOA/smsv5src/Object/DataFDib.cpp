...
CData * CDataFDib::CopyData(DBL movex, DBL movey)
{
	...
	if(m_pcDibCData != NULL){
		...
		_tcscpy_s(newfilename, m_pcDibCData->m_szImgFileName);
		diblibcreateimgfilename(newfilename);
		CopyFile(m_pcDibCData->m_szImgFileName, newfilename, FALSE);
		(pcnewData->m_pcDibCData)->SetFileName(newfilename);
		(pcnewData->m_pcDibCData)->InitElement();
