...
void CFSF1Font::OpenFileInitTable()
{
	...
	_stprintf_s(m_sFontfile, FNAMEMAX, _T("%s\\fsf1file.sf1"), szTmpPath);
	OpenFileInitTableMain();
}
void CFSF1Font::OpenFileInitTable0()
{
	TCHAR	szTmpPath[FNAMEMAX];

	senvgettmppath(szTmpPath);
	_stprintf_s(m_sFontfile, FNAMEMAX, _T("%s\\fsf1file0.sf1"), szTmpPath);
	OpenFileInitTableMain();
}
void CFSF1Font::OpenFileInitTable1()
{
	...
	senvgettmppath(szTmpPath);
	_stprintf_s(m_sFontfile, FNAMEMAX, _T("%s\\fsf1file1.sf1"), szTmpPath);
	OpenFileInitTableMain();
}
void CFSF1Font::OpenFileInitTable2()
{
	TCHAR	szTmpPath[FNAMEMAX];

	senvgettmppath(szTmpPath);
	_stprintf_s(m_sFontfile, FNAMEMAX, _T("%s\\fsf1file2.sf1"), szTmpPath);
	OpenFileInitTableMain();
}
void CFSF1Font::OpenFileInitTable3()
{
	TCHAR	szTmpPath[FNAMEMAX];

	senvgettmppath(szTmpPath);
	_stprintf_s(m_sFontfile, FNAMEMAX, _T("%s\\fsf1file3.sf1"), szTmpPath);
	OpenFileInitTableMain();
}
void CFSF1Font::OpenFileInitTable4()
{
	TCHAR	szTmpPath[FNAMEMAX];

	senvgettmppath(szTmpPath);
	_stprintf_s(m_sFontfile, FNAMEMAX, _T("%s\\fsf1file4.sf1"), szTmpPath);
	OpenFileInitTableMain();
}
void CFSF1Font::OpenFileInitTableMain()
{
	int		byte;

	FileNullSet();
	_tfopen_s(&m_fileFont, m_sFontfile, _T("r+b"));
	if(m_fileFont == NULL){
		return;
	}
	m_nUseMaxIdx = 1;
...
void CFSF1Font::FileNullSet()
{
	...
	_tfopen_s(&m_fileFont, m_sFontfile, _T("wb"));
	if (m_fileFont == NULL) {
		return;
	}
	for(i = 0; i < block; i++){
...
int CFSF1Font::DFileReadOpen(LPTSTR fname)
{
	_tfopen_s(&m_fileDFont, fname, _T("r+b"));
	if(m_fileDFont == NULL){
		return(FAIL);
	}
	return(0);
...
void CFSF1Font::DFileWriteOpen(LPTSTR fname)
{
	DFileNullSet(fname);
	_tfopen_s(&m_fileDFont, fname, _T("r+b"));
	m_nDataNo = 0;
...
void CFSF1Font::DFileNullSet(LPTSTR fname)
{
	BYTE	nullbuf[1024];
	int		block, rest, i;
	...
	block = (int)(FLIBDATATOP / 1024L);
	rest = (int)(FLIBDATATOP % 1024L);
	_tfopen_s(&m_fileDFont, fname, _T("wb"));
	for(i = 0; i < block; i++){
...
void CFSF1Font::PreBoldTrns(int hFno, DBL par)
{
	DBL		bsizex, bsizey;
	DBL		btopx, btopy;
	DBL		bmvx, bmvy;
...
void CFSF1Font::PostBoldTrns(int hFno, DBL par)
{
	DataSave(hFno);
...
void CFSF1Font::BmpFileOut(LPTSTR fname, int xsize, int ysize)
{
	FILE				*fp;
	...
	_tfopen_s(&fp, (LPTSTR)fname, _T("wb"));
...
void CFSF1Font::TrnsElementInit(DBL sx, DBL sy, DBL wd, DBL hi, DBL rag)
{
	m_dSX = sx;
	m_dSY = sy;
	m_dXTime = wd / FSF1MESH;
	m_dYTime = hi / FSF1MESH;
	m_dSin = sin(rag);
	m_dCos = cos(rag);
}
