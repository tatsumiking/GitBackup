...
void CARCFont::FontNameSet(LPTSTR pFont, int type)
{
	...
	len1 = _tcslen(_T("âpêî"));
	len2 = _tcslen(_T("SPOT"));
	if(_tcsnccmp(_T("âpêî"), pFont, len1) == 0){
		num = _ttoi(&(pFont[len1]));
		_stprintf_s(m_sBaseFname, FNAMEMAX, _T("%s\\FT2\\EI400\\E%03d.ft2"), fontpath, num);
	}
	else if(_tcsnccmp(_T("SPOT"), pFont, len2) == 0){
		num = _ttoi(&(pFont[len2]));
		_stprintf_s(m_sBaseFname, FNAMEMAX, _T("%s\\FT2\\SP70\\SP%03d.ft2"), fontpath, num);
	}
	//else if(strncmp("FT", pFont, 2) == 0){
	else{
		if(type == ARCFONT){
			len1 = _tcslen(_T("âpêî"));
			if(_tcsnccmp(_T("FT601"), pFont, len1) == 0){
				m_nNewType = ON;
			}
			_stprintf_s(m_sBaseFname, FNAMEMAX, _T("%s\\%s.ft2"), fontpath, pFont);
		}
		else{
			_stprintf_s(m_sBaseFname, FNAMEMAX, _T("%s\\%s.max"), fontpath, pFont);
		}
	}
	/*
	else{
		_stprintf_s(m_sBaseFname, "%s\\FT2\\WA\\%s.ft2", fontpath, pFont);
	}
	*/
}
...
int CARCFont::SF1TrnsDataLoad(int sjis)
{
	int		ret;
	DBL		rag;
...
int CARCFont::SF1TrnsDataLoadNON(int sjis)
{
	int		ret;
...
void CARCFont::SF1TrnsDataSave(int sjis)
{
	TrnsCopyFSF1FBuf();
	DataSave(sjis);
}

int CARCFont::DataLoad(int sjis)
{
	int	ku, ten;
	...
	if(m_nNewType == ON){
		if(sjis < 0x0100){
			if(0x021 <= sjis && sjis <= 0x07e){
				sjis = sjis - 0x0021 + 0x00859f;
			}
			if(0x00a1 <= sjis && sjis <= 0x00DF){
				sjis = sjis - 0x00a1 + 0x008640;
			}
		}
	}
	else{
	...
	if(sjis < 0x008140 || 0x00ffff < sjis){
		m_nDataLen = 0;
		return(0);
	}
	if(0x009fff < sjis && sjis < 0x00e021){
		m_nDataLen = 0;
		return(0);
	}
...
int CARCFont::ArcDataLoad(int ku, int ten)
{
	...
	if(m_nType == ARCFONT){
		...
		fread(&(arcidx.ku), sizeof(BYTE), 1, m_fileARC);
		fread(&(arcidx.ten), sizeof(BYTE), 1, m_fileARC);
...
int CARCFont::ArcDataSave(int ku, int ten)
{
	...
	if(m_nType == ARCFONT){
		...
		fseek(m_fileARC, idxseekp, 0);
		fwrite(&(arcidx.ku), sizeof(BYTE), 1, m_fileARC);
		fwrite(&(arcidx.ten), sizeof(BYTE), 1, m_fileARC);
...
int CARCFont::FontOpen(int ku, int ten)
{
	...
	len = _tcslen(m_sBaseFname);
	if(m_sBaseFname[len-7] == '6'
	&& m_sBaseFname[len-6] == _T('0')
	...
	_tfopen_s(&m_fileARC, m_sBaseFname, _T("rb"));
	if(m_fileARC == NULL){
		return(FAIL);
	}
	return(0);
...
{
	_tfopen_s(&m_fileARC, m_sBaseFname, _T("r+b"));
	if(m_fileARC == NULL){
		if(FontIdxInit() == FAIL){
			return(FAIL);
		}
...
int CARCFont::FontIdxInit()
{
	...
	_tfopen_s(&m_fileARC, m_sBaseFname, _T("wb"));
	if(m_fileARC == NULL){
		return(FAIL);
	}
	memset(buf, _T('\0'), HEADBYTE);
	_tcscpy_s(buf, m_sBaseFname);
...
void CARCFont::FaceNameSet(LPTSTR name)
{
	_tcscpy_s(m_sFaceName, name);
}
...
void CARCFont::ArcIdxSeekPointGet(int ku, int ten, long *seekp)
{
	...
	idx = (ku - 1) * 94 + (ten - 1);
	size = sizeof(TCHAR)+sizeof(TCHAR)+sizeof(long)+sizeof(long);
