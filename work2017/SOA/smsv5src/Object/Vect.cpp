...
void CVect::GdiplusFillDraw(CGdiPlusEx *pcGpe)
{
	...
	if (pdrwatrarcchk(atr0, atr1, atr2) == ON) {
		pcGpe->AddPathMoveTo(x0, y0);
	}
	else {
		pcGpe->AddPathMoveTo(x1, y1);
	}
	...
	for (pp = 0; pp < m_nPointCount;) {
		...
		if (pdrwatrbezechk(atr1, atr2, atr3) == ON) {
			if (p3 == 0) {
				pcGpe->AddPathLineTo(x2, y2);
				pcGpe->AddPathLineTo(x3, y3);
			}
			else if (p2 == 0) {
				pcGpe->AddPathLineTo(x2, y2);
			}
			else {
				pcGpe->AddPathBezierTo(x2, y2, x3, y3, x4, y4);
		}
			pp += 3;
		}
		else if (pdrwatrarcchk(atr1, atr2, atr3) == ON) {
			if (p2 != 0) {
				stAc.x1 = x1; stAc.y1 = y1;
				stAc.x2 = x2; stAc.y2 = y2;
				stAc.x3 = x3; stAc.y3 = y3;
				stBz = crossTransArcBezier(&stAc);
				pcGpe->AddPathBezierTo(stBz.x2, stBz.y2, stBz.x3, stBz.y3, stBz.x4, stBz.y4);
			}
			pp += 2;
		}
		else {
			if (pdrwatrcurvechk(atr1) == ON) {
				if (p1 == 0 && pdrwatrarcchk(atr0, atr1, atr2) == ON) {
					stAc.x1 = x0; stAc.y1 = y0;
					stAc.x2 = x1; stAc.y2 = y1;
					stAc.x3 = x2; stAc.y3 = y2;
					stBz = crossTransArcBezier(&stAc);
					pcGpe->AddPathBezierTo(stBz.x2, stBz.y2, stBz.x3, stBz.y3, stBz.x4, stBz.y4);
				}
				else {
					stFc.atr0 = atr0; stFc.x0 = x0; stFc.y0 = y0;
					stFc.atr1 = atr1; stFc.x1 = x1; stFc.y1 = y1;
					stFc.atr2 = atr2; stFc.x2 = x2; stFc.y2 = y2;
					stFc.atr3 = atr3; stFc.x3 = x3; stFc.y3 = y3;
					stBz = crossTransFreecurveBezier(&stFc);
					pcGpe->AddPathBezierTo(stBz.x2, stBz.y2, stBz.x3, stBz.y3, stBz.x4, stBz.y4);
				}
			}
			else {
	...
	pcGpe->PathCloseFigure();
}
void CVect::GdiplusPlgnDraw(CGdiPlusEx *pcGpe)
{
	int		pp, p0, p1, p2, p3, p4;
	int		atr0, atr1, atr2, atr3, atr4;
	DBL		x0, y0, x1, y1, x2, y2, x3, y3, x4, y4;
	...
	if (m_nPointCount == 0) {
		...
		scrndxymmtorltv(&x1, &y1);
		GdiplusPointDraw(pcGpe, atr1, x1, y1);
		return;
	}
	if (m_nPointCount == 2) {
		GetAtrDXY(0, &atr1, &x1, &y1);
		...
		scrndxymmtorltv(&x1, &y1);
		scrndxymmtorltv(&x2, &y2);
		pcGpe->DrawLine(x1, y1, x2, y2);
		GdiplusPointDraw(pcGpe, atr1, x1, y1);
		GdiplusPointDraw(pcGpe, atr2, x2, y2);
		return;
	}
	p0 = m_nPointCount - 1;
	p1 = 0;
	...
	scrndxymmtorltv(&x1, &y1);
	if (pdrwatrarcchk(atr0, atr1, atr2) == ON) {
		GdiplusPointDraw(pcGpe, 0, x0, y0);
	}
	else {
		GdiplusPointDraw(pcGpe, atr1, x1, y1);
	}
	...
	for (pp = 0; pp < m_nPointCount;) {
		...
		if (pdrwatrbezechk(atr1, atr2, atr3) == ON) {
			if (p3 == 0) {
				pcGpe->DrawLine(x1, y1, x2, y2);
				pcGpe->DrawLine(x2, y2, x3, y3);
				GdiplusPointDraw(pcGpe, atr2, x2, y2);
				GdiplusPointDraw(pcGpe, atr3, x3, y3);
			}
			else if (p2 == 0) {
				pcGpe->DrawLine(x1, y1, x2, y2);
				GdiplusPointDraw(pcGpe, atr2, x2, y2);
				...
				pcGpe->DrawBezier(x1, y1, x2, y2, x3, y3, x4, y4);
				pcGpe->DrawCLine(x1, y1, x2, y2);
				pcGpe->DrawCLine(x3, y3, x4, y4);
				GdiplusPointDraw(pcGpe, atr2, x2, y2);
				GdiplusPointDraw(pcGpe, atr3, x3, y3);
				GdiplusPointDraw(pcGpe, atr4, x4, y4);
			}
			pp += 3;
		}
		else if (pdrwatrarcchk(atr1, atr2, atr3) == ON) {
			if (p2 != 0) {
				...
				stAc.x3 = x3; stAc.y3 = y3;
				stBz = crossTransArcBezier(&stAc);
				pcGpe->DrawBezier(stBz.x1, stBz.y1, stBz.x2, stBz.y2, stBz.x3, stBz.y3, stBz.x4, stBz.y4);
				GdiplusPointDraw(pcGpe, atr2, x2, y2);
				GdiplusPointDraw(pcGpe, atr3, x3, y3);
			}
			pp += 2;
...
void CVect::GdiplusPointDraw(CGdiPlusEx *pcGpe, int atr, DBL dx, DBL dy)
{
	int ndot, hdot1, hdot2;
	DBL sx, sy, ex, ey;

	senvgetpointdotsize(&ndot);
	hdot1 = ndot / 2;
	hdot2 = ndot - hdot1;
	sx = dx - hdot1; sy = dy - hdot1;
	ex = dx + hdot2; ey = dy + hdot2;
	if (atr == 0) {
		pcGpe->DrawLPoint(sx, sy, ex, ey);
	}
	else {
		pcGpe->DrawCPoint(sx, sy, ex, ey);
	}
}
...
void CVect::GetFillRGB(LPINT rval, LPINT gval, LPINT bval)
{
	int penno;
...
void CVect::GetOutLineRGBW(LPINT rval, LPINT gval, LPINT bval, LPINT nthick)
{
	int penno;
	DBL		thick;
