...
BOOL CExcelLib::Open(CString *pcsFileName)
{
	...
	strcpy(szFileName, pcsFileName->GetBuffer(FNAMEMAX));
	m_pWorkbooks = NULL;
	m_pWorkbook = NULL;
	m_pWorksheets = NULL;
	m_pWorksheet = NULL;
...
// [名前] GetFileName
// [概要] ファイル名の取得
// [引数] LPSTR fname	// ファイル名
// [戻値] なし
// [更新日 / 更新者 / 更新内容]
// 2007/01/10 TAKE コメント作成
// --------------------------------------------------------------------------------
void CExcelLib::GetFileName(LPSTR fname)
{
	CString csPath;
	CString csFile;
	...
	strcpy(szPath, csPath.GetBuffer(RECMAX));
	strcpy(szFile, csFile.GetBuffer(RECMAX));
	sprintf(fname, "%s\\%s", szPath, szFile);
}
...
// 2007/01/10 TAKE コメント作成
// --------------------------------------------------------------------------------
void CExcelLib::GetCell(int col, int line, LPSTR str)
{
	COleVariant		vaResult;
	char	cellname[QRECMAX];
	CString	csStr;
	...
	strncpy(str, csStr.GetBuffer(RECMAX8), RECMAX8);
	str[RECMAX8-2] = '\0';
	str[RECMAX8-1] = '\0';
...
// 2007/01/10 TAKE コメント作成
// --------------------------------------------------------------------------------
void CExcelLib::SetCell(int col, int line, LPSTR str)
{
	COleVariant		vaResult;
	char	cellname[QRECMAX];
	CString	csStr;
...
BOOL CExcelLib::CheckType()
{
	...
	if(::CLSIDFromProgID(L"Excel.Application.8", &clsid) == S_OK){
		...
		AfxMessageBox("Microsoft Excelが必要です");
		return(FALSE);
	}
	return(TRUE);
}
...
{
	if(m_nExcelType == 8){
		if(!m_excel.CreateDispatch("Excel.Application.8")) {
			AfxMessageBox("Excel97をインストールしなおす必要があります");
			return(FALSE);
		}
	}else if(m_nExcelType == 9){
		if(!m_excel.CreateDispatch("Excel.Application.9")) {
			AfxMessageBox("Excel2000をインストールしなおす必要があります");
			return(FALSE);
		}
	}else if(m_nExcelType == 10){
		if(!m_excel.CreateDispatch("Excel.Application.10")) {
			AfxMessageBox("Excel2002をインストールしなおす必要があります");
			return(FALSE);
		}
	}else if(m_nExcelType == 11){
		if(!m_excel.CreateDispatch("Excel.Application.11")) {
			AfxMessageBox("Excel2003をインストールしなおす必要があります");
			return(FALSE);
		}
	}else if(m_nExcelType == 12){
		if(!m_excel.CreateDispatch("Excel.Application.12")) {
			AfxMessageBox("Excel2007をインストールしなおす必要があります");
			return(FALSE);
		}
	}else{
		return(FALSE);
...
// [名前] OpenFile
// [概要] ファイルのオープン処理
// [引数] LPSTR lpFileName	// ファイル名
// [戻値] BOOL
// [更新日 / 更新者 / 更新内容]
// 2007/01/10 TAKE コメント作成
// --------------------------------------------------------------------------------
BOOL CExcelLib::OpenFile(LPSTR lpFileName)
{
	short	num;
...
// [引数] int col		// カラム番号
// [引数] int line		// 行番号
// [引数] LPSTR cellname	// セル名称
// [戻値] なし
// [更新日 / 更新者 / 更新内容]
// 2007/01/10 TAKE コメント作成
// --------------------------------------------------------------------------------
void CExcelLib::CreateCelName(int col, int line, LPSTR cellname)
{
	char	colname[4];
	int		div, rest;
	...
	sprintf(cellname, "%s%d", colname, line);
}
