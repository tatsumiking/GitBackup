...
#include	"..\cmnsrc\DlgScrnE.h"
#include	"command.h"

#include <dlgs.h>       // for standard control IDs for commdlg

static char addfilter[] = "筆耕システム Files (*.iti)|*.iti||";
static char addsafixs[] = "iti";
static char crctfilter[] = "筆耕システム 貯込Files (*.rec)|*.rec||";
static char crctsafixs[] = "rec";
...
CCommand::CCommand(CScrollView* pcview)
{
	...
	m_pcCmdDBEdit = new CCmdDBEdit(pcview);
	...
	m_pcCmdMsur = new CCmdMsur(pcview);
	m_pcCmdPloter = new CCmdPloter(pcview);
	//m_pcCmdPrint = new CCmdPrint(pcview);
	//m_pcCmdCross = new CCmdCross(pcview);
	m_pcCmdKamon = new CCmdKamon(pcview);
	m_pcCmdMente = new CCmdMente(pcview);
	m_pcCmdSend = new CCmdSend(pcview);
	m_pcCmdFtp = new CCmdFtp(pcview);
	m_pcCmdEFtp = new CCmdEFtp(pcview);
	m_pcCmdSlct->m_pcCmdSWak = m_pcCmdSWak;
	m_nFirstFlag = TRUE;
	SetCrtCommand(NOOP);
	SetSubCommand(NOOP);
}
...
CCommand::~CCommand()
{
	...
	//delete(m_pcCmdStrgArc);
	delete(m_pcCmdEdit);
	delete(m_pcCmdSlct);
	delete(m_pcCmdDBEdit);
	delete(m_pcCmdFile);
...
void CCommand::SetSubCommand(int nSubCommand)
{
	...
	pcWnd->m_nCrtCommand = nSubCommand;
	pcWnd->SetCrtView((CTonView*)m_pcView);
...
UINT CCommand::Exec(UINT event, DBL dx, DBL dy)
{
	...
	//if(m_nCrtCommand == NOOP && m_nFirstFlag == TRUE){
	//	return(ENDEVENT);
	//}
	//m_nFirstFlag = FALSE;

	pcView = (CTonView*)m_pcView;
	pcApp = (CTonApp*)AfxGetApp();
	...
	if(event == INITEVENT){
		...
		sprintf(msg, "%02ld％ 実メモリ0x%06lx/0x%06lx 仮想ファイル0x%08lx/0x%08lx 仮想メモリ0x%08lx/0x%08lx",
			ms.dwMemoryLoad, 
			ms.dwAvailPhys, ms.dwTotalPhys,
			ms.dwAvailPageFile, ms.dwTotalPageFile,
			ms.dwAvailVirtual, ms.dwTotalVirtual);
		pcWnd->HelpTextOut(0, msg);
	}
#endif
	if(event == INITEVENT){
		if(m_nSubCommand != 0
		&& m_nSubCommand != ID_VIEW_PREVIEW
		&& m_nSubCommand != ID_SLCT_OBJ
		&& m_nSubCommand != ID_ZOOM_ALL
		&& m_nSubCommand != ID_ZOOM_IN
		&& m_nSubCommand != ID_ZOOM_OUT
		&& m_nSubCommand != ID_VIEW_REDISP
		&& m_nSubCommand != ID_VIEW_IMAGE){
			((CTonDoc*)m_pcDoc)->SaveUndo();
		}
		...
		case ID_FILE_ENVSET:
			...
			pcWnd->m_nCrtCommand = ID_FILE_ENVSET;
	...
	if(event == INITEVENT){
		if(m_nCrtCommand != ID_MAIN_SLCT
		&& m_nCrtCommand != ID_MAIN_ZOOM
		&& m_nCrtCommand != ID_MAIN_SUB){
	...
	case ID_OBJ_FIGU:
		...
		break;
	case ID_OBJ_RULE:
		ret = ExecRule(event, dx, dy);
		break;
	case ID_OBJ_BMP:
		ret = ExecDibC(event, dx, dy);
		break;
	case ID_OBJ_MSUR:
		ret = ExecMsur(event, dx, dy);
		break;
	case ID_TOOL_TRACE:
...
UINT CCommand::ExecFile(UINT event, DBL dx, DBL dy)
{
	...
	case ID_FILE_ADDDEL:
		...
		break;
	case ID_FILE_CRCTLOAD:
		if(event == INITEVENT){
			m_pcCmdFile->SetFilter(crctfilter, crctsafixs);
		}
		ret = m_pcCmdFile->ExecCrctLoad(event, dx, dy);
		break;
	case ID_FILE_CRCTSAVE:
		if(event == INITEVENT){
			m_pcCmdFile->SetFilter(crctfilter, crctsafixs);
		}
		ret = m_pcCmdFile->ExecCrctSave(event, dx, dy);
		break;
	case ID_FILE_CRCTDEL:
		if(event == INITEVENT){
			m_pcCmdFile->SetFilter(crctfilter, crctsafixs);
		}
		ret = m_pcCmdFile->ExecCrctDel(event, dx, dy);
		break;
	case ID_FILE_EPSSEND:
		ret = m_pcCmdSend->ExecEpsSend(event, dx, dy);
		break;
	case ID_FILE_MPRNSEND:
		ret = m_pcCmdSend->ExecMprnSend(event, dx, dy);
		break;
	case ID_FILE_MLTPRNFTP:
		ret = m_pcCmdFtp->ExecMltprnFtp(event, dx, dy);
		break;
	case ID_FILE_EMLTPRNFTP:
		ret = m_pcCmdEFtp->ExecMltprnFtp(event, dx, dy);
		break;
	case ID_FILE_PRNSEND:
		ret = m_pcCmdSend->ExecPrnSend(event, dx, dy);
		break;
...
UINT CCommand::ExecDBEdit(UINT event, DBL dx, DBL dy)
{
	UINT	ret;

	switch(m_nSubCommand){
	case ID_DBEDT_LOAD:
		ret = m_pcCmdDBEdit->ExecDBLoad(event, dx, dy);
		break;
	case ID_DBEDT_DELETE:
		ret = m_pcCmdDBEdit->ExecDBDelete(event, dx, dy);
		break;
	default:
		ret = ENDEVENT;
		break;
...
UINT CCommand::ExecStrg(UINT event, DBL dx, DBL dy)
{
	...
	case ID_MOJI_ROUND:
		...
		break;
	case ID_MOJI_FITING:
		ret = m_pcCmdMoji->ExecMojiFiting(event, dx, dy);
		break;
	case ID_MOJI_SIZE:
...
UINT CCommand::ExecFigu(UINT event, DBL dx, DBL dy)
{
	...
	case ID_FIGU_CRCL:
		...
		break;
	case ID_FIGU_PLGN:
		ret = m_pcCmdFigu->ExecPlgn(event, dx, dy);
		break;
	case ID_FIGU_FREE:
...
UINT CCommand::ExecRule(UINT event, DBL dx, DBL dy)
{
	UINT	ret;
	CTonDoc*	theDoc;
	...
	//	ret = m_pcCmdFigu->ExecFree(event, dx, dy);
	//	break;
	//case ID_RULE_SIZELIST:
	//	if(event == INITEVENT){
	//		theDoc->OffFlag(ACTIVEFLAG);			
	//	}
	//	ret = m_pcCmdFigu->ExecSizeList(event, dx, dy);
	//	if(ret == ENDEVENT){
	//		m_nCrtCommand = ID_MAIN_EDIT;
	//		m_nSubCommand = ID_EDIT_MOVE;
	//		ret = REDRAWEVENT;
...
UINT CCommand::ExecSpcl(UINT event, DBL dx, DBL dy)
{
	UINT	ret;
	...
	switch(m_nSubCommand){
	case ID_SPCL_SHADOW:
		ret = m_pcCmdSpcl->ExecShadow(event, dx, dy);		
		break;
	case ID_SPCL_OUTLINE:
		ret = m_pcCmdSpcl->ExecOutLine(event, dx, dy);		
		break;
	//case ID_RULE_OUTLINE:
	//	ret = m_pcCmdSpcl->ExecRuleOutLine(event, dx, dy);		
	//	break;
	case ID_SPCL_STRGSTRCT:
		ret = m_pcCmdSpclStrg->ExecStrgLineStruct(event, dx, dy);
		break;
	//case ID_SPCL_AREA:
	//	ret = m_pcCmdSpcl->ExecAreaSlct(event, dx, dy);
	//	break;
	case ID_SPCL_OMITOUTLINE:
		ret = m_pcCmdSpcl->ExecOutLineOmit(event, dx, dy);		
		break;
	default:
		ret = ENDEVENT;
		break;
...
UINT CCommand::ExecDibC(UINT event, DBL x, DBL y)
{
	...
	case ID_BMP_ROUND:
		...
		break;
	case ID_BMP_SAVE:
		ret = m_pcCmdDibC->ExecSave(event, x, y);
		break;
	case ID_BMP_JPGSAVE:
		ret = m_pcCmdDibC->ExecJpgSave(event, x, y);
		break;
	case ID_BMP_CLIP:
		ret = m_pcCmdDibC->ExecClip(event, x, y);
		break;
	case ID_BMP_BKLOAD:
		ret = m_pcCmdDibC->ExecBkLoad(event, x, y);
		break;
	case ID_BMP_BKRESET:
		ret = m_pcCmdDibC->ExecBkReset(event, x, y);
		break;
	default:
		ret = ENDEVENT;
		break;
...
UINT CCommand::ExecMsur(UINT event, DBL dx, DBL dy)
{
	UINT	ret;

	switch(m_nSubCommand){
	case ID_MSUR_XCREATE:
		ret = m_pcCmdMsur->ExecXCreate(event, dx, dy);
		break;
	case ID_MSUR_YCREATE:
		ret = m_pcCmdMsur->ExecYCreate(event, dx, dy);
		break;
	case ID_MSUR_ATRSET:
		ret = m_pcCmdMsur->ExecAtrSet(event, dx, dy);
		break;
	default:
		ret = ENDEVENT;
		break;
...
UINT CCommand::ExecCrdnt(UINT event, DBL dx, DBL dy)
{
	...
	case ID_CLR_SET:
		...
		break;
	case ID_CLR_BACKSET:
		ret = m_pcCmdClr->ExecClrBackSet(event, dx, dy);
		break;
	case ID_CLR_BACKRESET:
		ret = m_pcCmdClr->ExecClrBackReset(event, dx, dy);
		break;
	case ID_SORT_HEAD:
...
UINT CCommand::ExecWaku(UINT event, DBL dx, DBL dy)
{
	...
	case ID_WAKU_GRUP:
		...
		break;
	case ID_WAKU_PAPERDEL:
		ret = m_pcCmdWaku->ExecWakuPaperDel(event, dx, dy);
		break;
	case ID_WAKU_PAPER:
		ret = m_pcCmdWaku->ExecWakuPaperSet(event, dx, dy);
		break;
	case ID_WAKU_CREATE:
		ret = m_pcCmdSWak->ExecCreate(event, dx, dy);
		break;
	case ID_WAKU_INPUT:
		ret = m_pcCmdSWak->ExecAnyInput(event, dx, dy);
		break;
	case ID_WAKU_INSEKIJI:
		ret = m_pcCmdSWak->ExecAnyInSekiji(event, dx, dy);
		break;
	case ID_WAKU_SAVE:
	...
	case ID_KAMON_LOAD:
		...
		break;
	case ID_WAKU_CHANGE:
		ret = m_pcCmdSWak->ExecWakuChange(event, dx, dy);
		break;
	default:
		ret = ENDEVENT;
		break;
...
UINT CCommand::ExecMente(UINT event, DBL dx, DBL dy)
{
	UINT	ret;

	switch(m_nSubCommand){
	case ID_MENTE_CPTR_FOLDER:
		ret = m_pcCmdMente->ExecCopyTrnsFolder(event, dx, dy);
		break;
	case ID_MENTE_CPTR_FILE:
		ret = m_pcCmdMente->ExecCopyTrnsFile(event, dx, dy);
		break;
	default:
		ret = ENDEVENT;
		break;
