...
#ifndef __FSF1FONT_H
#define __FSF1FONT_H               // Prevent multiple includes

#include	"..\object\BoldLib.h"
...
public:
	...
	void CloseFile();
	void FileNullSet();
	int DFileReadOpen(LPSTR fname);
	void DFileWriteOpen(LPSTR fname);
	int DFileClose();
	void DFileNullSet(LPSTR fname);
	int  GetNextDataNo();
	void DataNoInc();
	...
	//太字生成処理関数
	void BoldTrns(int hFno, DBL par);
	void BmpFileOut(LPSTR fname, int xsize, int ysize);
