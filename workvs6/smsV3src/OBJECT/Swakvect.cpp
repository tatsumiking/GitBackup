...
void CSWakVect::Serialize(CArchive& ar)
{
	...
	if (ar.IsStoring())
	{
		...
		if(ver >= VER300){
			ar >> csStr;
			strcpy(m_szFildName, csStr.GetBuffer(RECMAX));
		}
...
void CSWakVect::GetFildName(LPSTR lpFildName)
{
	strcpy(lpFildName, m_szFildName);
}

void CSWakVect::SetFildName(LPSTR lpFildName)
{
	strcpy(m_szFildName, lpFildName);
}
...
void CSWakVect::SetString(char *pstr)
{
	int		flag;
	DBL		wd, hi;
	int		nFontType;
	...
	strcpy(m_sString, pstr);
	len = basesetcodetbl(pstr, codetbl);
	codetbl[len] = 0;

	nFontType = FontSelect(m_sFontName, flag);
	...
	while(1){
		...
		pcMojiVect->FontLoad(nFontType, sjis, code);
...
void CSWakVect::WakuSave(FILE * fp)
{
	...
	sprintf(buf, "\"SWAKVECT\",\"%s\",\"%s\", %4.1lf, %4.1lf, %4.1lf, %4.1lf, \"%s\", %ld, %ld, %d, %d, %lf, %lf, %lf, %lf, %lf, %d, %d, %d, %1.1lf\n",
...
void CSWakVect::WakuLoad(FILE *fp, LPSTR buf)
{
	char	*sp;
	char	numstr[HRECMAX];
	DBL		mmxsize, mmysize;
	...
	if(strncmp(szMatching, "SWAKVECT", 8) != 0){
		WakuCnvLoad(fp, buf);
		return;
	}
	...
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%d", &m_nOder);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%d", &m_nSpaceKind);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%lf", &m_dFlatPar);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%lf", &m_dSpacePar);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%lf", &m_dKanaPar);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%lf", &m_dThickPar);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%lf", &m_dSlopRag);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%d", &m_nYokoGumi);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%d", &m_nFixFlag);
	sp = basecsvstrdivcpy(sp, numstr);
	sscanf(numstr, "%d", &m_nMirror);
	//　2008.01.15 追加
	dCharCount = 0;
	if(sp[0] != '\n' && sp[1] != '\n'){
		sp = basecsvstrdivcpy(sp, numstr);
		if(isdigit((int)numstr[0])){
			sscanf(numstr, "%lf", &dCharCount);
		}
	}
...
void CSWakVect::WakuCnvLoad(FILE *fp, LPSTR buf)
{
	char	*sp;
	char	szMatching[RECMAX];
	char	szInStr[RECMAX];
