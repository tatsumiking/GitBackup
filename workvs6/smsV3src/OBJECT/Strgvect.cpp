...
void CStrgVect::Serialize(CArchive& ar)
{
	...
	else
	{
		...
		CString csString;
		ar >> csFontName;
		lstrcpy(m_sFontName, csFontName);
		ar >> m_nStrgFlag;
		ar >> m_nYokoGumi;
		ar >> m_nFlag3;
		ar >> m_nFlag4;
		ar >> csString;
		lstrcpy(m_sString, csString);
		ar >> m_dCharCount;
...
void CStrgVect::SetFontName(LPSTR sFname)
{
	strcpy(m_sFontName, sFname);
}

void CStrgVect::GetFontName(LPSTR sFname)
{
	strcpy(sFname, m_sFontName);
}
...
void CStrgVect::ChangeFont(LPSTR sFname)
{
	char	szStr[RECMAX];
...
void CStrgVect::SetString(char *pstr)
{
	int		len, idx;
	int		flag;
	UINT	sjis;
	...
	while(1){
		...
		pcMojiVect->FontLoad(m_nFontType, sjis, code);
...
void CStrgVect::GetString(char *pstr)
{
	strcpy(pstr, m_sString);
}

void CStrgVect::SetStringUnLoad(char *pstr)
{
	strcpy(m_sString, pstr);
}
...
void CStrgVect::FlagCheckMojiChange(int flag)
{
	...
	m_nFontType = FontSelect(sFontName, CheckTateYoko());
	...
	while(1){
		...
		if(pcMojiVect->CheckFlag(flag) == ON){
			sjis = pcMojiVect->m_nCode;
			code = pcMojiVect->m_nBaseCode;
			pcMojiVect->FontReload(m_nFontType, sjis, code);
		}
		pcMojiVect = pcMojiVect->GetNextMoji();
	}
}
...
void CStrgVect::TategakiElementInitWaku()
{
	...
	GetStrLen(&(SET.dSL));
	GetCharCount(&(SET.hcnt), &(SET.zcnt));
...
//P 全角と半角の文字の数を求める
void CStrgVect::GetCharCount(int* hcnt, int* zcnt)
{
	CMojiVect*	pcMojiVect;
	int		nYokoMojiIn;
...
int CStrgVect::FontSelect(LPSTR sFontName, int fTateGaki)
{
	char	sGJFaceName[FACEMAX];
	char	sFaceName[FACEMAX];
	char	sTategakiFaceName[FACEMAX];
	UINT	hc, idx;

	mojienvgetfonttypename(sFontName, &m_nFontType, sFaceName);
	if(m_nFontType == 0){
		strcpy(sFaceName, sFontName);
		m_nFontType = TT1FONT;
	}
	switch(m_nFontType){
	case ES0FONT:
	...
	case TT3FONT:
		...
		if(fTateGaki == TATEGAKI || fTateGaki == YOKOOKI){
			if(m_nFontType == TT3FONT){
				wsprintf(sTategakiFaceName, "%s", sFaceName);
			}
			else{
				wsprintf(sTategakiFaceName, "@%s", sFaceName);
			}
