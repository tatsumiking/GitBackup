...
DllExport void poutportnameset(LPSTR port)
{
	strcpy(sportname, port);
}

DllExport void poutportnameget(LPSTR port)
{
	strcpy(port, sportname);
}

DllExport void poutploternameset(LPSTR pltname)
{
	strcpy(splotername, pltname);
}

DllExport void poutploternameget(LPSTR pltname)
{
	strcpy(pltname, splotername);
}
...
DllExport void poutfilenoset(int fileno)
{
	...
	plotfilenameset(file);
	fclose(fp);
}

DllExport void poutdatanoset(int datano)
{
	char	exepath[FNAMEMAX];
	char	filename[FNAMEMAX];
	FILE	*fp;
...
void poutprmload()
{
	...
	senvgetexepath(exepath);
	sprintf(fname, "%s\\poutprm.env", exepath);
	...
	fgets(buf, RECMAX, fp);
	sscanf(buf, "%d", &poutdivmode);

	fgets(buf, RECMAX, fp);
	sscanf(buf, "%lf %lf %lf %lf", &rag, &mm, &mindiv, &maxdiv);
