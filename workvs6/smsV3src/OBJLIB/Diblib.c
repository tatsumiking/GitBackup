...
{
	char	tmpbmppath[FNAMEMAX];

	senvgetbtppath(tmpbmppath);
	sprintf(bmpfilename, "%s\\tbmp%04d.bmp", tmpbmppath, bmpfileno);
	bmpfileno++;
}
...
DllExport void diblibBmpSmallPictureCreate(LPSTR lpSrcFname, LPSTR lpDstFname, DBL xsize, DBL ysize)
{
	...
	dstxsize = (long)xsize;
	dstysize = (long)ysize;
	srcfp = fopen(lpSrcFname, "r+b");
	if(srcfp == NULL){
		return;
	}
	dstfp = fopen(lpDstFname, "w+b");
	if(dstfp == NULL){
		return;
	}
...
DllExport void diblibBmp256PlanePictureCreate(LPSTR lpSrcFname, LPSTR lpDstFname, DBL xsize, DBL ysize, COLORREF chkclr)
{
	...
	if(xsrcbyte > ONELINEMAX){
		MessageBox(NULL, "ファイルが大きすぎるため処理できません\n解像度をおとしてください\n", "警告", MB_OK);
        fclose(srcfp);
		fclose(dstfp);
		return;
	}
...
DllExport void diblibCmprimgToBmp(LPSTR dotfname, LPSTR bmpfname)
{
	...
	srcfp = fopen(dotfname, "r+b");
	if(srcfp == NULL){
		return;
	}
	dstfp = fopen(bmpfname, "w+b");
	if(dstfp == NULL){
		return;
	}
	ret = diblibGlobalLock();
	...
	fgets(rbuf, RECMAX, srcfp);
	sscanf(rbuf, "%s %d %d %d\n", tmp, &dotxlen, &dotylen, &dotdpi);
