...
DllExport int plgnfillinit()
{
	...
	//m_fpPlgn = fopen("c:\plgnfill.txt", "w");
...
void plgnfillarrypointset(int x, int y)
{
	if(nArryIdx >= PLGNPOINTMAX){
		if(nErrorFlag == OFF){
			MessageBox( NULL, "ì_ÇÃëçêîÇ™ëΩÇ¢ÇΩÇﬂìhÇËÇ¬Ç‘ÇµÇ≈Ç´Ç‹ÇπÇÒ", NULL, MB_OK );
			nErrorFlag = ON;
		}
		return;
	}
