...
UINT CCmdPoint::ExecSlopLegal(UINT event, DBL dx, DBL dy)
{
	...
	case SETEVENT:
		...
		if(m_pcVect != NULL && m_pcVect->CheckID() == BASEVECTID){
			...
			m_pcVect->PLDraw(m_pcDC);
			dx = m_dBX; dy = m_dBY;
			// 2001.09.28 立身 シフトを押した状態での斜め揃えは補助線に対して垂直にフィット
			if(((CTonView*)m_pcView)->CheckKeyFlag(MK_SHIFT) == TRUE){
				SlopLegalMainShift(m_pcVect, m_nPP);
			}
			else{
...
void CCmdPoint::MainAnyPointDelete()
{
	...
	if(m_nPP < m_nPP2){
		m_pcVect->GetAtrDXY(m_nPP, &atr, &dx, &dy);
		m_pcVect->SetAtrDXY(m_nPP, 0, dx, dy);
		dp = m_nPP+1;
		...
		dp = 0;
		m_pcBackVect->GetAtrDXY(m_nPP, &atr, &dx, &dy);
		m_pcBackVect->SetAtrDXY(m_nPP, 0, dx, dy);
...
void CCmdPoint::SlopLegalMainShift(CVect *pcVect, int pp)
{
	DBL		dx, dy;
	DBL		cx, cy, rag;
	DBL		sx, sy, ex, ey;
	...
	pcnt = 	pcVect->GetPointCount();
	pcVect->GetDXY(pp, &dx, &dy);
	ret = curveperpendicular(lnx.x1, lnx.y1, lnx.x2, lnx.y2, dx, dy, &xx1, &yy1);
	ret = curveperpendicular(lny.x1, lny.y1, lny.x2, lny.y2, dx, dy, &xx2, &yy2);


	len1 = hypot(dx - xx1, dy - yy1);
	len2 = hypot(dx - xx2, dy - yy2);	
	if(len1 < len2){
		pcVect->SetDXY(pp, xx1, yy1);
	}
	else{
		pcVect->SetDXY(pp, xx2, yy2);
	}
}
