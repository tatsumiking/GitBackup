...
#include "..\cmnsrc\dlgshdw.h"
#include "..\cmnsrc\dlgoutln.h"
...
extern "C" {
	...
	void scrngetmmtorltv(LPDBL dx, LPDBL dy); 
...
void CCmdSpcl::ShadowProg3()
{
	...
	for(i = 0; i <= nlen; i++){
		sprintf(m_szMsgBuf, "描画処理 %d ／ %d", i, nlen);
...
void CCmdSpcl::BmpFileOut(LPSTR fname, BYTE far *m_lpBits, int xbyte, int xsize, int ysize)
{
	FILE				*fp;
...
void CCmdSpcl::BitMapOutLineSet(BYTE *lpBits, BYTE *lpTBits, int xbyte, int xsize, int ysize)
{
	...
	for(y = 0; y < ysize; y++){
		if((y % 100) == 0){
			sprintf(m_szMsgBuf, "前処理 %d ／ %d", y, ysize);
			HlpMsgDsp(m_szMsgBuf);
		}
		//Dispatch();
		for(x = 0; x < xsize; x++){
...
void CCmdSpcl::BitMapTrace(BYTE *lpBits, BYTE *lpTBits, int xbyte, int xsize, int ysize)
{
	...
	for(y = 0; y < ysize; y++){
		if((y % 100) == 0){
			sprintf(m_szMsgBuf, "線生成処理 %d ／ %d", y, ysize);
			HlpMsgDsp(m_szMsgBuf);
		}
		//Dispatch();
		for(x = 0; x < xsize; x++){
