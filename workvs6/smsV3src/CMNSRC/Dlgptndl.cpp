...
CDialogPtnDelete::CDialogPtnDelete(CWnd* pParent /*=NULL*/)
	: CDialog(CDialogPtnDelete::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDialogPtnDelete)
	m_csPtnGrup = _T("");
	//}}AFX_DATA_INIT
	m_nFartFlag = TRUE;
	m_nCrtTop = 0;
...
void CDialogPtnDelete::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDialogPtnDelete)
...
BEGIN_MESSAGE_MAP(CDialogPtnDelete, CDialog)
	//{{AFX_MSG_MAP(CDialogPtnDelete)
...
/////////////////////////////////////////////////////////////////////////////
// CDialogPtnDelete メッセージ ハンドラ

void CDialogPtnDelete::OnSelchangeLstsptn() 
{
	m_nCrtTop = m_lstPtn.GetCurSel();
	Invalidate();
}

BOOL CDialogPtnDelete::OnInitDialog() 
{
...
void CDialogPtnDelete::OnLButtonDown(UINT nFlags, CPoint point) 
{
	int		sx, sy;
	int		idx;
...
void CDialogPtnDelete::DrawImage(CDC *pcDC, LPSTR lpBmpFname, int ndsx, int ndsy, int ndw, int ndh)
{
	CFile	cpFile;
	HANDLE	hDib;
	LPBYTE	lpDib;
...
void CDialogPtnDelete::OnSelchangeCmbptngrup() 
{
	int		idx;
...
void CDialogPtnDelete::ListInitSetItem()
{
	int		idx;
	char	ptnname[32];
...
void CDialogPtnDelete::ComboInitSetItem()
{
...
void CDialogPtnDelete::OnBtnPrepg() 
{
...
void CDialogPtnDelete::OnBtnNextpg() 
{
	m_nCrtTop = m_nCrtTop + 4;
	if(m_nCrtTop > (m_nSPtnMax-2)){
		m_nCrtTop = m_nSPtnMax-2;
