...
*/
	...
	pDialogInputNumPair->SetDialogTitle("倍率変更");
	pDialogInputNumPair->SetUnitText("倍", "倍");
	pDialogInputNumPair->SetTitleText("横倍率", "縦倍率");
...
UINT CCmdTrns::ExecTrnsSizeX(UINT event, DBL dx, DBL dy)
{
	...
	pDialogInputNum = new CDialogInputNum(m_pcView);
	pDialogInputNum->SetDialogTitle("横サイズ変更(横サイズ指定)");
	pDialogInputNum->SetUnitText("mm");
	pDialogInputNum->SetTitleText("横サイズ");
	xsize = maxx-minix;
...
UINT CCmdTrns::ExecTrnsSizeY(UINT event, DBL dx, DBL dy)
{
	...
	pDialogInputNum = new CDialogInputNum(m_pcView);
	pDialogInputNum->SetDialogTitle("縦サイズ変更(縦サイズ指定)");
	pDialogInputNum->SetUnitText("mm");
	pDialogInputNum->SetTitleText("縦サイズ");
	ysize = maxy-miniy;
...
/*	CTonDoc*	pcDoc;
	...
	pDialogInputNum = new CDialogInputNum(m_pcView);
	pDialogInputNum->SetDialogTitle("寸法サイズ変更(横サイズ指定)");
	pDialogInputNum->SetUnitText("mm");
	pDialogInputNum->SetTitleText("横サイズ");
	xsize = maxx-minix;
	pDialogInputNum->m_dInputNum = FLOOR100(xsize);
	...
	pDialogInputNum = new CDialogInputNum(m_pcView);
	pDialogInputNum->SetDialogTitle("寸法サイズ変更(縦サイズ指定)");
	pDialogInputNum->SetUnitText("mm");
	pDialogInputNum->SetTitleText("縦サイズ");
	ysize = maxy-miniy;
	pDialogInputNum->m_dInputNum = FLOOR100(ysize);
...
*/
	...
	pDialogInputNumPair = new CDialogInputNumPair(m_pcView);
	pDialogInputNumPair->SetDialogTitle("寸法サイズ変更");
	pDialogInputNumPair->SetUnitText("mm", "mm");
	pDialogInputNumPair->SetTitleText("横サイズ", "縦サイズ");
	xsize = maxx-minix;
	ysize = maxy-miniy;
...
UINT CCmdTrns::ExecTrnsSlopRagUd(UINT event, DBL dx, DBL dy)
{
	...
	pDialogInputNum = new CDialogInputNum(m_pcView);
	pDialogInputNum->SetDialogTitle("角度指定上下斜形変更");
	pDialogInputNum->SetUnitText("度");
	pDialogInputNum->SetTitleText("斜形角度");
	pDialogInputNum->m_dInputNum = 0;
	if(pDialogInputNum->DoModal() == IDOK){
...
void CCmdTrns::RoundDibC(CDibCData *pcDibCData, DBL cntrx, DBL cntry, DBL tsin, DBL tcos)
{
	...
	if(mode != -1){
		...
		hndle = ImgKitFileLoad(pcDibCData->m_szBmpFname, "", "");
		nhndle = ImgKitRotation(hndle, "", "", mode);
		diblibcreatebmpfile(pcDibCData->m_szBmpFname);
		ImgKitBmpFileSave(pcDibCData->m_szBmpFname, nhndle, "", "", 0);
		ret = ImgKitMemFree(hndle);	
		ret = ImgKitMemFree(nhndle);
		pcDibCData->DibReload();
		sx = sx - cntrx; sy = sy - cntry;
		ex = ex - cntrx; ey = ey - cntry;
		xx = sx * tcos - sy * tsin;
		yy = sx * tsin + sy * tcos;
		x1 = xx + cntrx; y1 = yy + cntry;
		xx = sx * tcos - ey * tsin;
		yy = sx * tsin + ey * tcos;
		x2 = xx + cntrx; y2 = yy + cntry;
		xx = ex * tcos - ey * tsin;
		yy = ex * tsin + ey * tcos;
		x3 = xx + cntrx; y3 = yy + cntry;
		xx = ex * tcos - sy * tsin;
		yy = ex * tsin + sy * tcos;
		x4 = xx + cntrx; y4 = yy + cntry;
		sx = min(x1, x2);
		sy = min(y1, y2);
		sx = min(sx, x3);
		sy = min(sy, y3);
		...
		ex = max(ex, x4);
		ey = max(ey, y4);
		pcDibCData->m_dDstSx = sx;
		pcDibCData->m_dDstSy = sy;
		pcDibCData->m_dDstWidth = ex - sx;
		pcDibCData->m_dDstHight = ey - sy;
...
void CCmdTrns::ArcTrnsVectMain()
{
	...
	cnt = SetDataListSelectID(SWAKVECTID, ACTIVEFLAG);
	pos = m_cPtrList.GetHeadPosition();
	while (pos != NULL)
	{
		pcData = m_cPtrList.GetNext(pos);
	...
	while (pos != NULL)
	{
		...
		// BEZEに変換
		pcCurveUtil->ConvertBezeVect((CVect*)pcData);
		ArcTrnsVect((CVect*)pcData, stdx, stdy);
	}
	delete(pcCurveUtil);
	//pcDoc = (CTonDoc*)(m_pcView->GetDocument());
	//pcDoc->MoveTrns(0.0, 0.0);
...
void CCmdTrns::DispSize()
{
	...
	sprintf(msg, "横%4.1lfmm 縦%4.1lfmm", m_dEX-m_dSX, m_dEY - m_dSY);
...
void CCmdTrns::Disp2PointSize()
{
	...
	theWnd = (CMainFrame*)theApp->GetMainWnd();
	len = hypot(m_dEX-m_dSX, m_dEY-m_dSY);
	sprintf(msg, " 長%4.1lf x%4.1lf y%4.1lf", len, m_dEX-m_dSX, m_dEY-m_dSY);
	theWnd->HelpTextOut(2, msg);
}
