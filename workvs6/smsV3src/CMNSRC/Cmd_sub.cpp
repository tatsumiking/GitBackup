...
:CCmdBase(pcview)
{
	...
	sprintf(fname, "%s\\defenv\\cmd_sub.env", exepath);
	if((fp = fopen(fname, "r")) != NULL){
		CrDelFgets(buf, HRECMAX, fp);
		sscanf(buf, "%lf", &m_dWakuSpace);
		CrDelFgets(buf, HRECMAX, fp);
		sscanf(buf, "%lf", &m_dCntrMM);
		fclose(fp);
	}
...
UINT CCmdSub::ExecSubGrid(UINT event, DBL dx, DBL dy)
{
	...
	pcDialogInputNum->SetDialogTitle("グリッドサイズ設定");
	pcDialogInputNum->SetUnitText("mm");
	pcDialogInputNum->SetTitleText("グリッド");
...
UINT CCmdSub::ExecSubAnyWaku(UINT event, DBL dx, DBL dy)
{
	...
	case INITEVENT:
		...
		m_pcDialogInputNum->SetDialogTitle("連続枠付け設定");
		m_pcDialogInputNum->SetUnitText("mm");
		m_pcDialogInputNum->SetTitleText("枠付間隔");
...
UINT CCmdSub::ExecSubCntrline(UINT event, DBL dx, DBL dy)
{
	...
	//m_dCntrMM = 5;
	pcDialogInputNum = new CDialogInputNum(m_pcView);
	pcDialogInputNum->SetDialogTitle("中心線の長さ設定");
	pcDialogInputNum->SetUnitText("mm");
	pcDialogInputNum->SetTitleText("長さ");
...
UINT CCmdSub::ExecCntrLineCood(UINT event, DBL dx, DBL dy)
{
	...
	pcDialogInputNumPair->SetDialogTitle("原点座標指定");
	pcDialogInputNumPair->SetUnitText("mm", "mm");
	pcDialogInputNumPair->SetTitleText("Ｘ座標", "Ｙ座標");
...
UINT CCmdSub::ExecCntrLineRltv(UINT event, DBL dx, DBL dy)
{
	...
	pcSysEnv->ScrnGetCntrXY(&cx, &cy);
	pcDialogInputNumPair = new CDialogInputNumPair(m_pcView);
	pcDialogInputNumPair->SetDialogTitle("原点座標相対移動");
	pcDialogInputNumPair->SetUnitText("mm", "mm");
	pcDialogInputNumPair->SetTitleText("Ｘ座標", "Ｙ座標");
...
UINT CCmdSub::ExecSubLineCood(UINT event, DBL dx, DBL dy)
{
	...
	pcDialogInputNumPair = new CDialogInputNumPair(m_pcView);
	pcDialogInputNumPair->SetDialogTitle("補助線座標指定");
	pcDialogInputNumPair->SetUnitText("mm", "mm");
	pcDialogInputNumPair->SetTitleText("Ｘ座標", "Ｙ座標");
	pcDialogInputNumPair->m_dInputNumX = FLOOR100(cx);
	pcDialogInputNumPair->m_dInputNumY = FLOOR100(cy);
...
UINT CCmdSub::ExecSubLineRltv(UINT event, DBL dx, DBL dy)
{
	...
	pcSysEnv->ScrnGetSubXY(&cx, &cy);
	pcDialogInputNumPair = new CDialogInputNumPair(m_pcView);
	pcDialogInputNumPair->SetDialogTitle("補助線相対移動");
	pcDialogInputNumPair->SetUnitText("mm", "mm");
	pcDialogInputNumPair->SetTitleText("Ｘ座標", "Ｙ座標");
	pcDialogInputNumPair->m_dInputNumX = 0;
	pcDialogInputNumPair->m_dInputNumY = 0;
...
UINT CCmdSub::ExecSubLineRag(UINT event, DBL dx, DBL dy)
{
	...
	pcDialogInputNum = new CDialogInputNum(m_pcView);
	pcDialogInputNum->SetDialogTitle("補助線角度指定");
	pcDialogInputNum->SetUnitText("度");
	pcDialogInputNum->SetTitleText("角度");
