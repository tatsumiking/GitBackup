...
extern "C" {
	...
	void diblibBmpSmallPictureCreate(LPSTR lpSrcFname, LPSTR lpDstFname, DBL xsize, DBL ysize);
	void diblibBmp256PlanePictureCreate(LPSTR lpSrcFname, LPSTR lpDstFname, DBL xsize, DBL ysize, COLORREF chkclr);
	void diblibBmpOnePlanePictureCreateES98(LPSTR lpSrcFname, LPSTR lpDstFname, COLORREF chkclr, int nLimit);
...
static char BASED_CODE szFilter[] = "画像ファイル (*.bmp;*.jpg)|*.bmp;*.jpg||";
...
:CCmdBaseBmp(pcview)
{
	...
	//strcpy(m_szEditProg, "Pbrush");
	strcpy(m_szEditProg, "mspaint");
	m_nImageXSize = 1024;
	m_nImageYSize = 768;
	m_nQuality = 50;
	m_nRotate = ON;
	...
	sprintf(fname, "%s\\defenv\\cmd_dibc.env", exepath);
	if((fp = fopen(fname, "r")) != NULL){
		CrDelFgets(buf, HRECMAX, fp);
		sscanf(buf, "%s", m_szEditProg);
		if(CrDelFgets(buf, HRECMAX, fp) != NULL){
			sscanf(buf, "%d %d", &m_nImageXSize, &m_nImageYSize);
		}
		else{
			m_nImageXSize = 1024;
			m_nImageYSize = 768;
		...
		if(CrDelFgets(buf, HRECMAX, fp) != NULL){
			sscanf(buf, "%d", &m_nQuality);
		}
		else{
			m_nQuality = 50;
		}
		if(CrDelFgets(buf, HRECMAX, fp) != NULL){
			sscanf(buf, "%d", &m_nRotate);
		}
		else{
			m_nRotate = ON;
		}
	...
	strcpy(m_szJpgInitPath, datpath);
	m_nJpgXSize = 1024;
	m_nJpgYSize = 768;
	m_nJpgQuality = 50;
	m_nJpgRotate = ON;
	m_szJpgFname[0] = '\0';
	senvgetexepath(exepath);
	sprintf(fname, "%s\\ImageOut.env", exepath);
	if((fp = fopen(fname, "r")) != NULL){
		if(CrDelFgets(m_szJpgInitPath, FNAMEMAX, fp) != NULL){
		}
		if(CrDelFgets(buf, HRECMAX, fp) != NULL){
			sscanf(buf, "%d %d", &m_nJpgXSize, &m_nJpgYSize);
		}
		if(CrDelFgets(buf, HRECMAX, fp) != NULL){
			sscanf(buf, "%d", &m_nJpgQuality);
		}
		if(CrDelFgets(buf, HRECMAX, fp) != NULL){
			sscanf(buf, "%d", &m_nJpgRotate);
		}
		fclose(fp);
	}
}
...
UINT CCmdDibC::ExecTrace(UINT event, DBL dx, DBL dy)
{
	...
	case DLGSETEVENT:
		if(m_nTrace == OFF){
			...
			if(::AfxMessageBox("元画像は削除しますか", MB_YESNO) == IDYES){
	...
	case SETEVENT:
		...
		if(m_pcDibCData != NULL){
			...
			if(pcDialogBmpRound->DoModal() == IDOK){
				...
				hndle = ImgKitFileLoad(m_pcDibCData->m_szBmpFname, "", "");
				nhndle = ImgKitRotation(hndle, "", "", mode);
				diblibcreatebmpfile(m_pcDibCData->m_szBmpFname);
				ImgKitBmpFileSave(m_pcDibCData->m_szBmpFname, nhndle, "", "", 0);
...
UINT CCmdDibC::ExecDotedit(UINT event, DBL dx, DBL dy)
{
	...
	case SETEVENT:
		if(m_nMouseSetCount == 0){
			...
			if(m_pcDibCData != NULL){
				SaveUndo();
				sprintf(OutProg, "%s \"%s\"", m_szEditProg, m_pcDibCData->m_szBmpFname);
				::WinExec(OutProg, SW_SHOW);
...
UINT CCmdDibC::ExecBmping(UINT event, DBL dx, DBL dy)
{
	...
	m_pcDibCData->SetBmpFname(fname);
	m_pcDibCData->DibLoad();
	m_pcDibCData->SetDstElement(sx, sy, wd, hi);
	m_pcDibCData->RenewMiniMax();

	if(::AfxMessageBox("元図形は削除しますか", MB_YESNO) == IDYES){
...
UINT CCmdDibC::ExecJpgSave(UINT event, DBL dx, DBL dy)
{
	...
	if(ret == IDOK){
		...
		strcpy(m_szJpgFname, csFileName.GetBuffer(FNAMEMAX));
		strcpy(m_szJpgInitPath, m_szJpgFname);
		baseaddsafix(m_szJpgFname, "jpg");
	}
...
UINT CCmdDibC::ExecClip(UINT event, DBL dx, DBL dy)
{
	...
	if(m_pcDibCData == NULL){
		::AfxMessageBox("選択状態の画像がありません", MB_OK);
		return(ENDEVENT);
	}
	if(m_pcDataList == NULL){
		::AfxMessageBox("選択状態の図形データがありません", MB_OK);
		return(ENDEVENT);
	}
	m_pcDoc->DeleteData(m_pcDibCData);
...
BOOL CCmdDibC::LoadDibCData(LPSTR lpPath)
{
	HANDLE		hndle;
	CTonDoc*	pcDoc;
	char	fname[FNAMEMAX];
	...
	CMyFileDialog	*pcFileDialog = new CMyFileDialog(TRUE, NULL, "*.bmp;*.jpg", NULL, szFilter, m_pcView);
	if(lpPath != NULL){
		pcFileDialog->SetInitPath(lpPath);
	}
	pcFileDialog->SetTitle("ビットマップファイル読込");
	if(pcFileDialog->DoModal( ) == IDOK){
		CString cpStr = pcFileDialog->GetPathName();	
		strcpy(fname, cpStr.GetBuffer(FNAMEMAX));
		len = strlen(fname);
		if((fname[len-3] == 'j' || fname[len-3] == 'J')
		&& (fname[len-2] == 'p' || fname[len-2] == 'P')
		&& (fname[len-1] == 'g' || fname[len-1] == 'G')){
...
void CCmdDibC::SetTraceClr()
{
	...
	m_pcDibCData->GetBmpFname(srcfname);
...
void CCmdDibC::TraceMain()
{
	...
	cpFile.SeekToBegin();
	cpFile.ReadHuge(m_lpDib1, filesize);
...
void CCmdDibC::BmpFileOut(LPSTR fname, BYTE far *m_lpBits, int xbyte, int xsize, int ysize)
{
	FILE				*fp;
...
void CCmdDibC::BitMapTarceSetPlgn(BYTE *lpBits, BYTE *lpTBits, int xbyte, int x, int y)
{
	...
	SetPlgnInit();
	senvgetexepath(exepath);
	sprintf(fname, "%s\\pointdel.lst", exepath);
	fp = fopen(fname, "w");
	rsx = x; rsy = y;
	tsx = x; tsy = y;
	tx = x; ty = y;
...
BOOL CCmdDibC::SaveImageFile()
{
	...
	scrngetmmsize(&xsize, &ysize);
	if(m_szJpgFname[0] == '\0'){
		sprintf(fname, "%s\\%s", m_pcApp->m_szSavePath,  m_pcApp->m_szSaveName);
	}else{
		strcpy(fname, m_szJpgFname);
	}
	...
	if(mode != -1){
		hndle = ImgKitFileLoad(fname, "", "");
		nhndle = ImgKitRotation(hndle, "", "", mode);
		ret = ImgKitBmpFileSave(fname, nhndle, "", "", 0);
		ret = ImgKitMemFree(hndle);
		ret = ImgKitMemFree(nhndle);
	}
	...
	if(jpgfname[0] != '\0'){
		hndle = ImgKitFileLoad(fname, "", "");
		ret = ImgKitJpegFileSave(jpgfname, hndle, "", "", m_nQuality, 0);
	}
...
{
	char	fname[FNAMEMAX];
	char	cpyfname[FNAMEMAX];
	FILE	*fp;
	char	buf[RECMAX8+16];
	LPSTR	csvsp[RECMAX];

	if(m_pcApp->m_szArgv1[0] == '\0'){
		return;
	}
	strcpy(fname, m_pcApp->m_szArgv1);
	fp = fopen(fname, "r");
	if(fp == NULL){
		return;
	}
	...
	sprintf(cpyfname, "%s\\%s", m_pcApp->m_szSavePath,  m_pcApp->m_szSaveName);
	basechangesafix(cpyfname, "txt");
...
void CCmdDibC::SaveDocument()
{
	...
	senvgetdatpath(datpath);
	sprintf(fname, "%s\\%s", datpath,  m_pcApp->m_szSaveName);
	basechangesafix(fname, "iti");
	...
	basechangesafix(fname, "bin");
	m_pcView->PrintEnvSave(fname);
}

void CCmdDibC::SaveCrtEnv(LPSTR fname)
{
	FILE	*fp;
	DBL		xmmsize, ymmsize;
