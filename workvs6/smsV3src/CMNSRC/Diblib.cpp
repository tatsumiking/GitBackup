...
CDC *CDibLib::CreateDIBDC(int xsize, int ysize)
{
	...
	memset((LPSTR)(lpbi), 0, m_dwFileSize);
...
void CDibLib::GetBitImage()
{
	...
	sprintf(bmpfname, "%s\\tmpsys$$.bmp", btppath);
...
void CDibLib::BmpFileOut(LPSTR fname, BYTE far *m_lpBits, int xbyte, int xsize, int ysize)
{
	FILE				*fp;
