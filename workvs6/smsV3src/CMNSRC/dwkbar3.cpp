...
void CDialogWakuBar3::GetItemElement()
{
	...
	strcpy(pcSysEnv->m_szStrg, m_csInText.GetBuffer(RECMAX));
...
void CDialogWakuBar3::GetLocate()
{
	...
	dwidth = atof(m_csSizeX);
	if(dwidth < 0){
		m_csSizeX.Format("%1.1lf", fabs(dwidth));
		dwidth = fabs(dwidth);
		UpdateData(FALSE);
	}
	dhight = atof(m_csSizeY);
	if(dhight < 0){
		m_csSizeY.Format("%1.1lf", fabs(dhight));
		dhight = fabs(dhight);
		UpdateData(FALSE);
	}

	dmmsx = atof(m_csStartX);
	dmmsy = atof(m_csStartY);
...
void CDialogWakuBar3::SetItemElement()
{
	...
	m_csInText = _T(pcSysEnv->m_szStrg);
...
void CDialogWakuBar3::SetLocate()
{
	...
	m_csSizeX.Format("%1.1lf", dwidth);
	m_csSizeY.Format("%1.1lf", dhight);
	...
	m_csStartX.Format("%1.1lf", dmmsx);
	m_csStartY.Format("%1.1lf", dmmsy);
...
void CDialogWakuBar3::TateYokoFlagBottonSet(int ftategaki)
{
	...
	switch(ftategaki){
	case YOKOGAKI:
		m_btnTateYoko.SetWindowText("横書");
		break;
	case TATEGAKI:
		m_btnTateYoko.SetWindowText("縦書");
		break;
	case YOKOOKI:
		m_btnTateYoko.SetWindowText("横置");
		break;
	case TATEOKI:
		m_btnTateYoko.SetWindowText("縦置");
		break;
	}
}
...
void CDialogWakuBar3::OnChkShadow() 
{
	...
	UpdateData(TRUE);
	xsize = atof(m_csSizeX);
	ysize = atof(m_csSizeY);
	if(xsize == 0 || ysize == 0){
		m_nShadow = FALSE;
		UpdateData(FALSE);
		return;
