...
void CDialogWakuDelete::WakuDraw(CDC *pcDC, DBL sx, DBL sy, LPSTR wakufname)
{
...
void CDialogWakuDelete::WakuImageDraw(CDC *pcDC, DBL sx, DBL sy, LPSTR initfname, long nSize)
{
	CFile	cpFile;
	LPBYTE	lpDib;
	LPBYTE 	lpInfo;
...
void CDialogWakuDelete::WakuBoxsDraw(CDC *pcDC, DBL sx, DBL sy, LPSTR wakufname)
{
	FILE	*fp;
	char	buf[RECMAX8+16];
	char	*sp;
...
void CDialogWakuDelete::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	while(1){
		...
		&& (sy-WAKUSPACE) <= point.y && point.y <= (sy+WAKUSIZE)){
			...
			if(AfxMessageBox("枠を削除してよろしいですか", MB_OKCANCEL) == IDCANCEL){
...
void CDialogWakuDelete::GetWakuSize(char *wenvfile, DBL *wakuxsize, DBL *wakuysize)
{
	FILE	*fp;
	char	buf[RECMAX];
