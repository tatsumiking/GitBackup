...
BOOL CCmdMoji::MojiLoadMain()
{
	...
	if(ret == IDOK){
		strcpy(fontname, cpDialogMojiLoad->m_csFontName.GetBuffer(RECMAX));
		strcpy(codestr, cpDialogMojiLoad->m_csMojiCode.GetBuffer(QRECMAX));
		hc = (codestr[0] & 0x00ff);
...
BOOL CCmdMoji::MojiSaveMain()
{
	...
	if(ret == IDOK){
		strcpy(fontname, cpDialogMojiSave->m_csFontName.GetBuffer(RECMAX));
		strcpy(codestr, cpDialogMojiSave->m_csMojiCode.GetBuffer(QRECMAX));
		hc = (codestr[0] & 0x00ff);
		if(baseiskanji(hc) == ON){
			lc = codestr[1];
			sjis = ((hc << 8) & 0x00ff00) | (lc & 0x00ff);
...
BOOL CCmdMoji::MojiDelLoadMain()
{
	...
	cpDialogMojiLoad = new CDialogMojiLoad(m_pcView);
	cpDialogMojiLoad->SetDialogTitle("“o˜^íœ");
	ret = cpDialogMojiLoad->DoModal();
	if(ret == IDOK){
		strcpy(m_szFontName, cpDialogMojiLoad->m_csFontName.GetBuffer(RECMAX));
		strcpy(codestr, cpDialogMojiLoad->m_csMojiCode.GetBuffer(QRECMAX));
		hc = (codestr[0] & 0x00ff);
		if(baseiskanji(hc) == ON){
			lc = codestr[1];
