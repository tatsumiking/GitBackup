...
void CDialogGaijiSlct::InitNameCmb()
{
	...
	sprintf(szGjFName, "%s\\GAIJITBL.LST", szExePath);
	fp = fopen(szGjFName, "r");
	if(fp == NULL){
		return;
	}
	while(1){
		if(fgets(szBuf, QRECMAX, fp) == NULL){
			break;
		}
		sp = szBuf;
...
{
	UpdateData(TRUE);
	strcpy(m_szStr, m_csText[idx].GetBuffer(QRECMAX));
	CDialog::OnOK();
}
